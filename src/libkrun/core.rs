...
fn build_libkrun_config(
    ...
    let base_cmdline = {
        ...
        // Note: rootflags=dax=inode uses per-inode DAX mode, allowing virtiofs to
        // control DAX per-file via FUSE_ATTR_DAX flag (disabled for executables)
        format!(
            "reboot=k panic=-1 panic_print=0 nomodule console=ttyS0,115200 {} \
             rootfstype=virtiofs rw rootflags=dax=inode no-kvmapf \
    ...
    let base_cmdline = {
        ...
        format!(
            "reboot=k panic=-1 panic_print=0 nomodule console=hvc0 {} \
             rootfstype=virtiofs rw rootflags=dax=inode no-kvmapf {} {}{} init={}",
