...
run $SHELL_CMD "mkdir -p $TEST_TMP/zigproj && cd $TEST_TMP/zigproj && echo 'const std = @import(\"std\"); pub fn main() void { std.debug.print(\"ok\", .{}); }' > main.zig"
run $SHELL_CMD "export ZIG_GLOBAL_CACHE_DIR=$TEST_TMP/zig-cache && cd $TEST_TMP/zigproj && zig run main.zig" | grep -q ok
lang_ok
